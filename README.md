...
[![Build Status](https://img.shields.io/travis/quantisnetevo/insight-api/master.svg?branch=master)](https://travis-ci.org/quantisnetevo/insight-api)
...
```bash
npm install -g @quantisnetevo/quantisnetcore-node
quantisnetcore-node create mynode
cd mynode
quantisnetcore-node install @quantisnetevo/insight-api
quantisnetcore-node start  # to also start the service
```
...
To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `quantisnetcore-node.json` with:
...
With all the configuration options available: https://github.com/quantisnetevo/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
```
{
  "result":"[[\"proposal\",{\"end_epoch\":1519848619,\"name\":\"ghijklmnopqrstuvwxyz01234567891519097947\",\"payment_address\":\"yik5HAgVAgjH1oZKjcDfvcf22bwBNbSYzB\",\"payment_amount\":10,\"start_epoch\":1519097947,\"type\":1,\"url\":\"https://www.quantisnetcentral.org/p/test_proposal_1519097947\"}]]",
  "error":null,
  "id":78637
}
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in quantisnetd.
- The endpoint for `/peer` is no longer relevant connection to quantisnetd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking quantisnetd.
...
Feel free to dive in! [Open an issue](https://github.com/quantisnetevo/insight-api/issues/new) or submit PRs.
