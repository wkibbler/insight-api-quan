...
describe('Currency', function() {
  ...
  var quantisnetCentralData = {
    ...
    exchange_rates: {
      quantisnet_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_quantisnet: 0.01388998
    }
  };

  it.skip('will make live request to quantisnet central', function(done) {
    var currency = new CurrencyController({});
    var req = {};
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.quantisnet_usd);
        (typeof response.data.quantisnet_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(quantisnetCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.quantisnet_usd);
        response.data.quantisnet_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
